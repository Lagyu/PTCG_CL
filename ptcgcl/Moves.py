...
def torment():
    ...
    if "CANNOT_USE_MOVE_0" in Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_SC_1")][0]:
        Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_SC_1")][0]["CANNOT_USE_MOVE_1"] = move_id
    else:
        Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_SC_1")][0]["CANNOT_USE_MOVE_0"] = move_id
    return True
...
def x_blast(move_id=100):
    if move_id == 100:
        move_id = Perform.ask_opp_move_id(0)
    if "CANNOT_USE_MOVE_0" in Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_SC_0")][0]:
        Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_SC_0")][0]["CANNOT_USE_MOVE_1"] = move_id
    else:
        Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_SC_0")][0]["CANNOT_USE_MOVE_0"] = move_id
    return True
    ...
    if no_str == "all":
        for i in range(8):
            if len(Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_P_1")][i]) == 0:
                ...
                Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_1")][i][0] = \
                    Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_1")][i][0] + damage_amount
    else:
        bench_id_list = Perform.choose_opponents_pokemons_on_board(True, int(no_str))
        for i in range(len(bench_id_list)):
            Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_1")][bench_id_list[i]][0] = \
                Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_1")][bench_id_list[i]][0] + damage_amount
    return True
...
def only_10_left(mine: bool, opponents: bool):
    if mine:
        ...
        Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_0")][0][0] = my_max_hp_list[0] - 10
    if opponents:
        opp_max_hp_list = Check.calc_opponents_all_pokemons_max_hp()
        Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_1")][0][0] = opp_max_hp_list[0] - 10
    return True


def poisoned(nx_int=1):
    Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_SC_1")][0]["POISONED"] = nx_int
    return True


def asleep(coin_int=1):
    Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_SC_1")][0]["ASLEEP"] = coin_int
    return True
...
def paralized(coin=True):
    if coin:
        if Perform.flip_coin():
            Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_SC_1")][0]["PARALYZED"] = 1
            return True
        else:
            return False
...
def heal_damage(bench_id: int, heal_amount: int):
    if Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_0")][bench_id][0] < heal_amount:
        Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_0")][bench_id][0] = 0
    else:
        Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_0")][bench_id][0] = Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_0")][bench_id][0] - 30
    return True


def deal_damage_to_opp_bench(no_str: str, damage_amount: int, battle_bool=False):
    if no_str == "all":
        if "BENCH_BARRIER" in Board.BOARD_ELEM[Board.BOARD_DIC.index("BOARD_SC_1")][0]:
            Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_1")][0][0] = \
                Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_1")][0][0] + damage_amount
        else:
            for i in range(8):
                if len(Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_P_1")][i]) == 0:
                    pass
...
def discard_my_battle_pokemons_energy(discard_energy_str_list: list):
    for i in len(discard_energy_str_list):
        ...
        if energy_str_i in parsed_energy:
            ...
            for k in range(Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_E_0")][0]):
                if Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_E_0")][0][k]["name"] == name_discard:
                    Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_E_0")][0].pop(k)
...
def discard_my_battle_pokemons_all_energy(discard_energy_str="all"):
    if discard_energy_str == "all":
        Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_E_0")][0] = []
    else:
...
def flip_coin_and_deal_more_damage_num(num=0):
    if Perform.flip_coin():
        Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_1")][0][0] = \
        Board.BOARD_ELEM[Board.BOARD_DIC.index("POKEMON_D_1")][0][0] + num
