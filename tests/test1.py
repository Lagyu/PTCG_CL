...
#    print(len(ptcgcl.Board().BOARD_ELEM[ptcgcl.Board.Board().BOARD_DIC.index("DECK_0")]))
...
def test_battle_starting():
    ...
    for i in range(7):
        ptcgcl.Perform.draw()
    print("Drawn 7 cards!")
    print("Hand is: " + str(ptcgcl.Board().BOARD_ELEM[ptcgcl.Board.Board().BOARD_DIC.index("HAND_0")]))
    k = 0
    while not ptcgcl.Check.check_basic_in_hand():
        if k < 100:
            print("Mulligan!")
            print(ptcgcl.Board().BOARD_ELEM[ptcgcl.Board.Board().BOARD_DIC.index("HAND_0")])
            ptcgcl.Perform.mulligan()
            print("New hand is: " + str(ptcgcl.Board().BOARD_ELEM[ptcgcl.Board.Board().BOARD_DIC.index("HAND_0")]))
            k = k + 1
        else:
    ...
    hand_name_str = ""
    for i in range(len(ptcgcl.Board().BOARD_ELEM[ptcgcl.Board.Board().BOARD_DIC.index("HAND_0")])):
        hand_name_str = hand_name_str + ", " + ptcgcl.Board().BOARD_ELEM[ptcgcl.Board.Board().BOARD_DIC.index("HAND_0")][i]["name"]
    print("Hand(Name only) is" + hand_name_str)

    ptcgcl.Perform.put_prize()
    print("placed 6 prize cards.")
    print("Deck count: "+str(len(ptcgcl.Board().BOARD_ELEM[ptcgcl.Board.Board().BOARD_DIC.index("DECK_0")])))
'''
